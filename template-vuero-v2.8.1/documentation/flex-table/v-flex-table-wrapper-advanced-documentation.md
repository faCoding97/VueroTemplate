### واکشی داده های همگام (پیشرفته)

بیشتر اوقات، داده ها باید به صورت ناهمزمان از سرور واکشی شوند.
خوشبختانه، ویژگی `data` کامپوننت `<VFlexTableWrapper />` می‌تواند **async function** را بپذیرد که هر بار که یکی از ویژگی‌های مرتب‌سازی، فیلتر کردن یا صفحه‌بندی تغییر می‌کند فعال می‌شود.
      ...
      <VFlexTable rounded clickable @row-click="onRowClick">
        ...
        <template #body-cell="{ row, column }">
          ...
          <template v-if="column.key === 'actions'">
            <VAction>
              {{ row?.id === openedRowId ? 'مخفی کردن جزئیات' : 'مشاهده جزئیات' }}
            </VAction>
          </template>
        </template>
        ...
        <template #body-row-post="{ row }">
          ...
          <template v-if="row?.id === openedRowId">
            <div class="is-block p-4 my-2 is-rounded">
              <div class="dark-text mb-4 is-size-4">جزئیات {{ row?.name }}'</div>
