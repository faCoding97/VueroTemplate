    ...
    feedback: z
      .array(
        z.object({
          title: z
            .string()
            .min(10, 'عنوان تجربه شما باید حداقل 10 کاراکتر باشد'),
...
const handleSignup = handleSubmit(async (values) => {
  ...
  if (values.email !== 'awesome@cssninja.io') {
    setFieldError('email', 'این ایمیل قبلا گرفته شده است! نکته: از awesome@cssninja.io استفاده کنید')
    scrollTo('#email')
    loading.value = false
    return
  }
...
<template>
  ...
  >
    ...
    <VField
      id="passwordCheck"
      v-slot="{ field }"
       label="رمز عبور جدید خود را تأیید کنید"
    >
    ...
    >
      <VControl>
        <VSelect>
          ...
          <VOption :value="{ timezone: 'asia/tokyo', label: 'توکیو' }">
            asia
          </VOption>
          <VOption :value="{ timezone: 'america/new_york', label: 'نیویورک' }">
            america
          </VOption>
          <VOption :value="{ timezone: 'australia/sydney', label: 'سیدنی' }">
            australia
          </VOption>
        </VSelect>
        <p
    ...
    <VField
      id="allergens"
      v-slot="{ field }"
      label="آلرژن های خود را انتخاب کنید"
    >
      <VControl>
        <VCheckbox
          class="pl-0"
    ...
    >
      <VControl>
        <VSwitchBlock> موافقت میکنم با <a href="#">شرایط و ضوابط</a> </VSwitchBlock>
